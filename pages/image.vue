    ...
    <div>
      ...
      <div>
        ...
        <el-select v-model="paramData.style_preset" placeholder="Select">
          <el-option
            v-for="item in options.style_preset"
            :key="item"
            :label="item"
            :value="item"
          >
      ...
      <div>
        ...
        <div v-if="IMAGE && IMAGE.length > 0 && IMAGE[0]?.isQual === 'N'">
          <el-button
            type="primary"
            :disabled="IMAGE_LOADING_QUAL"
            @click="onClickImageLoadQuality(IMAGE[0]?.timestamp)"
            >해상도 보정</el-button
          >
          <el-button
            type="primary"
            :disabled="IMAGE_LOADING_QUAL"
            @click="onClickImageLoadStep(IMAGE[0]?.timestamp)"
            >퀄리티 보정</el-button
          >
          <el-button
            type="primary"
            :disabled="IMAGE_LOADING_QUAL"
            @click="onClickImageLoadStep2(IMAGE[0]?.timestamp)"
            >이미지 변형</el-button
          >
        </div>
...
export default {
  ...
  methods: {
    ...
    ...mapActions([
      'ACTION_IMAGE_BOT',
      'ACTION_IMAGE_QUALITY',
      'ACTION_IMAGE_QUALITY_STEP',
    ]),
    ...
    onLoadImage(v) {
      if (v)
        return (
          process.env.VUE_APP_API.replace('/chatGpt.php', '') +
          v.replace('./', '/')
        )
    },
    onLoadMotion() {},
